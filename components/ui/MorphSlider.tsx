...
export const MorphSlider = forwardRef<MorphSliderHandle, MorphSliderProps>(
  function MorphSlider(
    ...
    return (
      <div
        className={`relative h-full w-full overflow-hidden bg-[#0c0c0e] select-none ${className}`.trim()}
        style={
          {
      ...
      >
        <div
          className="absolute inset-0 cursor-grab outline-none active:cursor-grabbing focus-visible:shadow-[inset_0_0_0_2px_rgba(255,255,255,0.7)]"
          ref={containerRef}
          role="group"
        ...
        {showCaptions && hasCaptions ? (
          <div
            aria-live="polite"
            className="pointer-events-none absolute bottom-[22px] left-[22px] z-[2] grid max-w-[70%]"
          >
            {items.map((item, i) =>
              item.caption ? (
                <span
                  aria-hidden={i === index ? undefined : true}
                  className={`pointer-events-none inline-block [grid-area:1/1] [justify-self:start] rounded-[10px] bg-[rgba(10,10,12,0.42)] px-[14px] py-[8px] text-[15px] font-semibold tracking-[0.01em] text-white backdrop-blur-[8px] [transition:opacity_var(--ms-swap)_cubic-bezier(0.16,1,0.3,1),transform_var(--ms-swap)_cubic-bezier(0.16,1,0.3,1),filter_var(--ms-swap)_cubic-bezier(0.16,1,0.3,1)] ${
                    i === index
        ...
        {showControls ? (
          <div className="pointer-events-none absolute inset-x-0 top-1/2 z-[3] flex -translate-y-1/2 justify-between px-4">
            <button
              ...
              className="pointer-events-auto inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border border-white/20 bg-[rgba(12,12,14,0.4)] text-white backdrop-blur-md transition-transform duration-200 hover:scale-105 hover:bg-[rgba(24,24,28,0.6)] active:scale-95"
              onClick={handlePrev}
              type="button"
            >
              ←
        ...
        {showIndicators ? (
          <div
            aria-label="Slides"
            className="absolute inset-x-0 bottom-[18px] z-[3] flex items-center justify-center gap-2"
            role="tablist"
          >
            {items.map((item, i) => (
              <button
                ...
                className={`h-2 cursor-pointer rounded-full [transition:width_var(--ms-dot)_cubic-bezier(0.16,1,0.3,1),background-color_var(--ms-dot)_ease] ${
