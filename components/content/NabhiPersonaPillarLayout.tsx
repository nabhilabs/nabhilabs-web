...
type NabhiPersonaPillarLayoutProps = {
...
const architectureCards: Array<{
...
}> = [
  {
    ...
    copy: "Connect the tools teams already use—tickets, decks, CRMs, wikis—without another migration theatre.",
    icon: Database,
    features: [
  ...
  },
  {
    ...
    copy: "Memory that updates from real outcomes—not static snapshots that rot in a drive.",
    icon: Search,
    features: [
  ...
  },
  {
    ...
    copy: "Answers in the language of the work—not generic chat that invents when context is thin.",
    icon: Layers3,
    features: [
...
export function NabhiPersonaPillarLayout({ page }: NabhiPersonaPillarLayoutProps) {
  const product = getProductByPath(page.path);
  const keywordList = product
    ? productKeywordsCsv(product)
    : page.primaryKeyword;

  const [problemSection, implementationSection, comparisonSection] = page.sections;
      ...
      <section className="bg-[#f2f4f0] px-6 py-14 md:px-10 md:py-20">
        <div className="mx-auto max-w-[95rem] rounded-3xl border border-[#d8e0d5] bg-white p-6 shadow-[0_12px_48px_rgba(15,28,19,0.06)] md:p-10 lg:p-14">
          <div className="grid gap-12 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:items-center">
            <ChallengeAssetFrame
              alt="Scattered institutional knowledge connecting into one intelligence layer — Nabhi Persona"
              priority
              src="/assets/nabhi_persona_challenge_asset.webp"
            />
          ...
          <div className="mt-12 grid gap-8 border-t border-[#e8ece6] pt-10 md:grid-cols-3 md:gap-10">
            {problemSection?.body.map((paragraph, index) => (
      ...
      <section className="bg-[#f7f6f1] px-6 py-14 md:px-10 md:py-20">
        <div className="mx-auto max-w-[95rem]">
          ...
          <p className="mt-5 max-w-3xl text-sm leading-7 text-[#4a5b4e]">
            {implementationSection?.body[0]}
          </p>

          <ul className="mt-10 grid gap-6 lg:grid-cols-3">
            {architectureCards.map(({ title, copy, icon: Icon, features }) => (
              <li
          ...
          <p className="mt-8 max-w-3xl text-sm leading-7 text-[#4a5b4e]">
            {implementationSection?.body[1]}
          </p>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-[#4a5b4e]">
            {implementationSection?.body[2]}
          </p>
        </div>
      </section>
      ...
      <section className="bg-[#f2f4f0] px-6 py-14 md:px-10 md:py-20">
        <div className="mx-auto max-w-[95rem]">
          ...
          <p className="mt-4 max-w-xl text-sm leading-7 text-[#4a5b4e]">
            Nabhi Persona sits alongside knowledge platforms, RAG engineering,
            and operational workflows. Explore the paths that usually ship
            together.
          </p>
      ...
      <section className="bg-[#f7f6f1] px-6 py-14 md:px-10 md:py-20">
        <div className="mx-auto grid max-w-[95rem] gap-10 lg:grid-cols-2 lg:items-center">
          <div>
            ...
            <div className="mt-6 space-y-5 text-sm leading-7 text-[#4a5b4e]">
              {comparisonSection?.body.map((paragraph) => (
          ...
          <div className="relative aspect-[4/3] overflow-hidden rounded-3xl border border-[#d8e0d5] shadow-[0_20px_60px_rgba(15,28,19,0.12)]">
            <Image
              alt="Nabhi Persona — institutional knowledge as active intelligence"
              className="object-cover object-[68%_center]"
              fill
              sizes="(max-width: 1024px) 100vw, 50vw"
              src="/assets/nabhi_persona_wallpaper.webp"
            />
            <div
              aria-hidden
